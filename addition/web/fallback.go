...
func supportsTavilySearch(model string) bool {
	return !globals.IsGeminiModel(model) &&
		!globals.IsXAIModel(model) &&
		!globals.IsOpenAIResponsesNativeWebModel(model)
}

func CanUseSearchTool(enable bool, model string, toolCallsSupported bool) bool {
	return enable &&
		toolCallsSupported &&
		supportsTavilySearch(model) &&
		strings.TrimSpace(globals.SearchApiKey) != ""
}

func ShouldUseFallbackSearch(enable bool, model string, toolCallsSupported bool) bool {
	return enable &&
		!toolCallsSupported &&
		supportsTavilySearch(model) &&
		strings.TrimSpace(globals.SearchApiKey) != ""
...
func ToFallbackSearched(message []globals.Message, group string, cache *redis.Client) []globals.Message {
