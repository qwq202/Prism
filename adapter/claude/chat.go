...
const (
	...
	interleavedThinkingBetaValue = "interleaved-thinking-2025-05-14"
)
...
func (c *ChatInstance) GetChatHeaders(props *adaptercommon.ChatProps) map[string]string {
	...
	if props != nil && props.Thinking != nil && props.Tools != nil && len(*props.Tools) > 0 {
		headers["anthropic-beta"] = interleavedThinkingBetaValue
	}
...
func normalizeToolChoice(choice *interface{}, thinking interface{}) *ToolChoice {
		...
		if rawType == "function" {
			if function, ok := v["function"].(map[string]interface{}); ok {
				if name, ok := function["name"].(string); ok && strings.TrimSpace(name) != "" {
					...
					result = &ToolChoice{Type: "tool", Name: &name}
				}
			}
		}

		if result == nil && rawType != "" {
	...
	if thinking != nil && (result.Type == "any" || result.Type == "tool") {
...
func (c *ChatInstance) GetChatBody(props *adaptercommon.ChatProps, stream bool) *ChatBody {
	return &ChatBody{
		...
		ToolChoice:  normalizeToolChoice(props.ToolChoice, props.Thinking),
...
func (c *ChatInstance) CreateStreamChatRequest(props *adaptercommon.ChatProps, hook globals.Hook) error {
	...
	err := utils.EventScanner(&utils.EventScannerProps{
		...
		Headers: c.GetChatHeaders(props),
